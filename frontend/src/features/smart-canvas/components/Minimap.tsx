...
import { computeNodeBounds } from "../core/layout";
...
export function Minimap({
  ...
  const bounds = useMemo(() => computeNodeBounds(nodes), [nodes]);
  ...
  const viewX =
    (-viewport.x / viewport.scale - bounds.minX) * scale;
  const viewY =
    (-viewport.y / viewport.scale - bounds.minY) * scale;

  return (
    <div
      className="absolute bottom-20 right-4 border border-[var(--border)] bg-[var(--bg)]/80 p-1"
