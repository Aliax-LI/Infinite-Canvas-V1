...
class SqlitePromptLibraryRepository(PromptLibraryRepository):
    ...
    def load(self) -> dict[str, Any]:
        ...
        try:
            row = conn.execute("SELECT document_json FROM prompt_libraries WHERE id = 1").fetchone()
            if not row:
                return {}
    ...
    def save(self, data: dict[str, Any]) -> None:
        ...
        try:
            with transaction(conn):
                conn.execute(
                    """
                    INSERT INTO prompt_libraries (id, document_json) VALUES (1, ?)
