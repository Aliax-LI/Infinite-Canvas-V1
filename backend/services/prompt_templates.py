...
from backend.config import PROMPT_TEMPLATE_CANDIDATES

PROMPT_TEMPLATE_EN: dict[str, dict[str, str]] = {}


def prompt_template_markdown_path() -> str:
    for path in PROMPT_TEMPLATE_CANDIDATES:
        if path.is_file():
            return str(path)
    return ""
...
def parse_prompt_template_markdown(text: str) -> list[dict]:
    templates = []
...
def builtin_prompt_templates() -> list[dict]:
    try:
