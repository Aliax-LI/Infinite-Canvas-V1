...
class VersionedStaticFiles(StaticFiles):
    """StaticFiles that injects ?v= cache-busting params at request time.

    HTML and CSS source files in git stay clean (no ?v=), avoiding noise in
    git diff.  The version is computed from VERSION + file mtime, exactly
    like the old sync_static_html_versions() did — but in memory only.
    """
        ...
        if full_path.endswith(('.html', '.css')):
            try:
                with open(full_path, 'r', encoding='utf-8') as f:
                    content = f.read()
                transformed = versioned_static_html(content)
                media = ("text/html" if full_path.endswith('.html') else "text/css") + "; charset=utf-8"
                return Response(transformed, media_type=media,
...
def current_desktop_build_id():
    build_file = os.path.join(BASE_DIR, "DESKTOP_BUILD_ID")
    try:
...
def app_info():
    ...
    return {
        ...
        "is_electron": os.getenv("INFINITE_CANVAS_ELECTRON", "").lower() in ("1", "true", "yes"),
...
def update_connectivity_probe(name: str):
    ...
    raise HTTPException(status_code=404, detail="应用内热更新已废弃")
...
def fetch_github_latest_release(timeout: float = 5.0) -> Dict[str, Any]:
    ...
    try:
        ...
        release_url = str(payload.get("html_url") or f"{GITHUB_REPO_URL}/releases").strip()
        return {
            "ok": bool(version),
            "version": version,
...
def check_update():
    """检测 GitHub Releases 最新版本；桌面端只提示并跳转下载，不再热更新本地文件。"""
    current = current_app_version()
    latest_release = fetch_github_latest_release(timeout=5.0)
    latest: Dict[str, Any] = {}
